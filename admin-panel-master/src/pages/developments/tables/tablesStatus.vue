...
<div class="table w-full p-2">
 ...
 <td class="p-2 border-r">{{item.id}}</td>
                    ...
                    <td class="p-2 border-r">{{item.hari}}</td>
                    <td class="p-2 border-r">{{item.deskripsi}}</td>
                    <td>
                        <a v-bind:href="'#/admin/formstatus/'+ item.id" class="bg-blue-500 p-2 text-white hover:shadow-lg text-xs font-thin">Edit</a>
...
<script>
  ...
  export default {
    data() {
     ...
     methods:{
       delet (e) {
           axios.delete(`http://192.168.10.102:5000/status/`+e)
    .then(response => {
         this.$swal(response.data);
         axios.get(`http://192.168.10.102:5000/status`)
    .then(response => {
